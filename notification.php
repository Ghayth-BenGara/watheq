...
<html lang = "en">   
  <head>
    ...
    <title>Notifications | سوق واثق</title>
        ...
        <div class = "row">
                      				...
                      				<div class = "row">
										<div class = "col-lg-12">
											<div class = "section-top-border">
                            					<h3 class = "mb-30 title_color">Notifications</h3>
                            					<a href = " javascript:void(0)" id = "remove_notif" onclick = "return deleteNotification(`<?php echo $_SESSION['email'];?>`)"><i class = "lnr lnr-trash"></i></a>
										    	<?php showListeNotification($_SESSION['email']);?>  
											</div>
										</div>
									</div>
								</div>
			...
			<div class = "col-lg-4">
            	<div class = "blog_right_sidebar">
					...
					<aside id = "load" class = "load"></aside>
        		</div>
      		</div>
    	</div>
  	</div>
    ...
    <script>
        $(document).ready(function(){
			...
			$(document).on('click','#more_notif',function(){
				var last = $(this).data('nid');
				var email = "<?php echo $_SESSION['email'];?>";
				$('#more_notif').html("Chargement..");
				$.ajax({
					url:'php/fonctions.php',
					method:'POST',
					data:{
						last:last,email:email,loadNotif:'load'
					},
					dataType:'text',
					success:function(data){
						if(data.trim() != ''){
							$('#remove_row').remove();
							$('#notif_item').append(data.trim());
						}
					}
				});
			});
